...
import { parseShowType, ShowType } from '@/features/watch/types';
...
async function loadMovieDetail(id: string, type: ShowType): Promise<MovieDetailPayload> {
    return MovieDetail.getMovieDetail(id, type);
}

export async function generateMetadata({ params }: PageParams): Promise<Metadata> {
    const showType = parseShowType(params.type);
    if (!showType) {
        return {
            title: 'Show not found | CineView',
            description: 'Sorry, this title could not be found.',
        };
    }

    const data = await loadMovieDetail(params.id, showType);
...
export default async function MovieDetailPage({ params, searchParams }: PageParams) {
    const showType = parseShowType(params.type);
    const tmdbId = Number(params.id);

    if (!showType || !Number.isFinite(tmdbId)) {
        notFound();
    }

    const countryParam = searchParams?.country;
    const countryValue = Array.isArray(countryParam) ? countryParam[0] : countryParam;
    ...
    return (
        <main className="relative min-h-screen bg-gradient-to-br from-gray-950 via-black to-gray-950 text-gray-100">
            ...
            <section className="max-w-6xl mx-auto px-4 py-10">
                ...
                <WhereToWatch type={showType} tmdbId={tmdbId} country={selectedCountry} />
